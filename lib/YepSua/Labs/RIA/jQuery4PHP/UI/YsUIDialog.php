...
class YsUIDialog extends YsUICore {
  ...
  public function registerOptions() {
    return   array(//options
               ...
               //extras
               '_canMaximize' => array('key' => 'canMaximize', 'is_quoted' => false),
               '_canMinimize' => array('key' => 'canMinimize', 'is_quoted' => false),
  ...
  */
  ...
  public static function restore($jquerySelector = null){
    $jquery = new YsJQueryCore();
    $jquery->setEvent(self::$uiEvent);
    $jquery->addArgument(new YsArgument(YsUIConstant::RESTORE_METHOD));
    if($jquerySelector !== null){
      $jquery->setSelector($jquerySelector);
    }
    return $jquery;
  ...
  public static function minimize($jquerySelector = null){
    $jquery = new YsJQueryCore();
    $jquery->setEvent(self::$uiEvent);
    $jquery->addArgument(new YsArgument(YsUIConstant::MINIMIZE_METHOD));
    if($jquerySelector !== null){
      $jquery->setSelector($jquerySelector);
    }
    return $jquery;
  ...
  public static function maximize($jquerySelector = null){
    $jquery = new YsJQueryCore();
    $jquery->setEvent(self::$uiEvent);
    $jquery->addArgument(new YsArgument(YsUIConstant::MAXIMIZE_METHOD));
    if($jquerySelector !== null){
      $jquery->setSelector($jquerySelector);
    }
    return $jquery;
